...
def mapping(args):
    ...
    rt_inv, cam_origin, tree_data, rgb_data = args
    ...
    for h in range(0,2080):
        ...
        for w in range(0,2080):
            ...
            min = 99
            for scale in drange(-6,6,'0.1'): # z value doesnt exceed 10 within 3d points of building  TODO (before -8, 8)
                
                # scale_refine = scale * np.array([1.5 , 1., 1.5]) # TODO scale adjustment
                ray = scale * ray_hat + idx[0:3]
                dist, ind = kdtree.query([ray])

                if (dist < min and dist < 0.13700424361012217):  # max value yields from compare3Dpoints.py   # TODO modified dist condition
                    ...
                    if w < 600 and w >= 1480:
                        ...
                        tmp = "experiment2[{h},{w}] : {d} and color {c}".format(h=h, w=res_w, d= tree_data[ind[0][0]], c=res2[h,res_w])
...
if __name__ == "__main__":
    ...
    with open("./result/no_experiment_result.txt", "w") as f:
        for exp in experiment_result:
            f.write(exp)
    
    cv2.imshow("reverse_noray" , res)
    cv2.imshow("reverse_norayexperiment", res2)
    cv2.imwrite("reverse_noray.jpg", res)
    cv2.imwrite("reverse_noray_experiment.jpg", res2)
    cv2.waitKey()
