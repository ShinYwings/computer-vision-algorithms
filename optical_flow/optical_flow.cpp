...
int main (int argc, char **argv)
{
    ...
    putText(prevFrame, "[ADD POINT: MOUSE LEFT CLICK], [START TRACKING : MOUSE RIGHT CLICK]", Size(10,20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0,0,0), 2, LINE_AA);
    while (!isStartSet)
    {
        imshow ("Extracted Frame", prevFrame);
        cv::setMouseCallback ("Extracted Frame", onMouse, (void*)&prevFrame);
        cv::waitKey (30);
    }
	...
	while ( !stop )
    {
        ...
        for(int i=0; i<start.size() ; i++)
        {
            ...
            // tracking
            cv::circle (frame, finish_xy, 7, cv::Scalar (255, 0, 0));
            cv::line (mask, start_xy, finish_xy, cv::Scalar (0, 0, 255), 3);
        }
