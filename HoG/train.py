...
def read_images(pos_files, neg_files):
    ...
    for img_file in pos_files:
        ...
        # features = hog(gray, orientations=9, pixels_per_cell=(8, 8), cells_per_block=(2, 2), block_norm="L2-Hys", transform_sqrt=False, feature_vector=True)
        features = hog.hog(gray)
    ...
    for img_file in neg_files:
        ...
        for win in windows:
            # features = hog(win, orientations=9, pixels_per_cell=(8, 8), cells_per_block=(2, 2), block_norm="L2-Hys", transform_sqrt=False, feature_vector=True)
            features = hog.hog(win)
            features = features.ravel()
            neg_count += 1
            X.append(features)
            Y.append(0)
...
def hard_negative_mine(f_neg, winSize, winStride):
    ...
    for imgfile in f_neg:
        ...
        for (x, y, im_window) in sliding_window(gray, winSize, winStride):
            # features = hog(im_window, orientations=9, pixels_per_cell=(8, 8), cells_per_block=(2, 2), block_norm="L2-Hys", transform_sqrt=False, feature_vector=True)
            features = hog.hog(im_window)
            features = features.ravel()
...
# clf1 = SVM()
# clf2 =SVM()
# clf1 = svmpy.SVMTrainer(svmpy.Kernel.linear(), 0.01)
