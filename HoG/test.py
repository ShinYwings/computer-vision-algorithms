...
clf = joblib.load('person_final_original.pkl')
...
def read_images(f_pos, f_neg):
    ...
    for imgfile in f_neg:
        ...
        cropped = crop_centre(img)
        gray = cv2.cvtColor(cropped, cv2.COLOR_BGR2GRAY)
        features = hog(gray, orientations=9, pixels_per_cell=(8, 8), cells_per_block=(2, 2), block_norm="L2-Hys", feature_vector=True)
        # features = hog.hog(gray)
        # features = features.ravel()
...
accuracy = float(true_positives+false_negatives) / (true_positives+false_negatives+true_negatives+false_positives)
